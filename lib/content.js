...
function apiBase() {
  ...
  let base = 'https://wuya.oksja.cn/content-api/'
  // #ifdef H5
  base = '/content-api/'
  // #endif
  return base
...
export async function submitComment(postId, content, nickname = '微信用户', parentId = 0) {
  const body = await callApi('post/comment.json', {
    method: 'POST',
    data: { postId: Number(postId), parentId: Number(parentId) || 0, content: content.trim(), nickname }
  })
...
export const media = url => {
  ...
  if (normalized.startsWith('/profile/')) {
    // #ifdef MP-WEIXIN
    return `https://wuya.oksja.cn${normalized}`
    // #endif
    // #ifdef H5
    return normalized
    // #endif
...
const DEVICE = 'crowlook:device-key'
export const favorites = () => uni.getStorageSync(FAV) || []
function deviceKey() {
  let value = uni.getStorageSync(DEVICE)
  if (!value) {
    value = `device_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 14)}`
    uni.setStorageSync(DEVICE, value)
  }
  return value
}
export async function toggleFavorite(post) {
  let items = favorites()
  ...
  try {
    const response = await callApi('post/favorite.json', { method: 'POST', data: { postId: Number(post.id), deviceKey: deviceKey(), active } })
    post.fav_count = response.favorite_count
    return active
  } catch (error) {
    uni.setStorageSync(FAV, previous)
    throw error
  }
}

export function shareUrl(postId) {
  const base = String(config.share_base || 'https://wuya.oksja.cn').replace(/\/$/, '')
  return `${base}/#/pages/articleDetail/articleDetail?id=${postId}`
}
