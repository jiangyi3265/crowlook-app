<template><view v-if="modelValue" class="overlay" @click="close"><view class="sheet" @click.stop><view class="sheet-header"><button class="close" aria-label="关闭评论" @click="close"><CrowIcon name="cross"/></button><text>写评论</text><button class="publish" :disabled="submitting" @click="publish">{{ submitting ? '提交中' : '发表' }}</button></view><textarea v-model="draft" placeholder="说两句吧（审核通过后公开显示）" :focus="true" maxlength="1000"/></view></view></template>
<script setup>
import { ref, watch } from 'vue'
import CrowIcon from './CrowIcon.vue'
import { submitComment } from '../lib/content'

const props = defineProps({ modelValue: Boolean, postId: [String, Number] })
...
const publish = async () => {
  ...
  try {
    await submitComment(props.postId, content)
    draft.value = ''
