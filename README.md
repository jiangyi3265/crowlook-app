...
本仓库使用 Vue 3 与 UniApp 实现 Crowlook 用户端，可构建为 H5 和微信小程序。应用包含首页、作品分类、品牌内容、文章/图集详情、搜索、收藏、评论展示、客服服务和门店地图等 12 个页面路由；本地数据快照用于首屏与离线兜底，其余公开内容通过只读 API 按需加载。

点赞、搜索历史、评论草稿及顾客追加的案例照片仅保存在用户设备，不会向参考站点提交内容。H5 开发与预览脚本包含 API 和视频代理；微信小程序构建则使用 UniApp 对应平台适配器。
...
微信开发者工具导入 `dist/build/mp-weixin`。正式发布前需在 `manifest.json` 配置自己的微信小程序 AppID，并为实际使用的 API、图片和视频域名设置合法域名。H5 正式部署还需要提供与本地脚本等效的 `/reference-api/` 和 `/reference-video` 代理。
...
参与 Crowlook 品牌用户端开发，使用 Vue 3 与 UniApp 构建兼容 H5、微信小程序的作品展示应用，完成内容分类、图集详情、搜索收藏、客服与门店等跨端功能，并设计公开 API 的只读代理与本地数据兜底方案。
