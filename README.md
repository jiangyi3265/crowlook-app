...
点赞会在用户设备保留快捷列表，同时写入 Crowlook 后端形成统一统计；搜索历史和顾客追加的案例照片保存在用户设备。评论及回复会提交到后端并进入管理后台审核，通过后显示在作品详情页；失败时草稿保留在本地。首页文案、客服二维码、预约入口、搜索推荐和门店信息均由管理后台统一配置，H5 与微信小程序共享同一套数据。
...
微信开发者工具导入 `dist/build/mp-weixin`。正式发布前需在 `manifest.json` 配置自己的微信小程序 AppID，并为实际使用的 API、图片和视频域名设置合法域名。H5 正式部署需要把 `/content-api/` 和 `/profile/` 转发到 Crowlook 后端，并保留 `/reference-video` 媒体代理；未显式配置接口时，微信小程序构建默认使用线上 Crowlook API。
